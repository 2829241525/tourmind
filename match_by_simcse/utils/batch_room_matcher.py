...
DEFAULT_CONFIG = {
    ...
    # 认证相关配置
    "cookie": "admintools_user_session=MTc0MTg1MDM4OHxzQ0xSc1RRX2ZRalJlbW9ObXVpOEpXbFQxRWF6MTh2UkNsczZocWJHUjRtX1ZpTmxIUHRUQkJSNUtiQTVncWpSeVQtU3pBMVNXd1k9fJlMHFJhX3dv5H7dnWY1k4ad4IChzN6mPDaHBJykJsJA",
    ...
    # 数据文件配置
    "supplier_mapping_file": "/home/maxon/disk2/roomMatch/room_match/match_by_simcse/data/supplier_type.xlsx",
    "mapping_file": "/home/maxon/disk2/roomMatch/room_match/match_by_simcse/data/mapping.xlsx",
    ...
    def fetch_room_data(self, hotel_id: str) -> Optional[Dict]:
        ...
        while retries < self.config["max_retries"]:
            try:
                ...
                retries += 1
                self.logger.warning(f"获取数据失败 (重试 {retries}/{self.config['max_retries']}): {str(e)}")
...
class RoomTypeMatcher:
    ...
    def __init__(self, api_url: str, model_path: str, config: Dict):
        ...
        # 初始化数据清洗器
        mapping_path = os.path.join(config["base_dir"], "match_by_simcse/data/mapping.xlsx")
        ...
        try:
            ...
            self.logger.info(f"加载模型: {model_full_path}")
            self.predictor = SimCSEPredictor(model_full_path)
    ...
    def _load_supplier_mapping(self) -> Dict[str, str]:
        ...
        try:
            # 读取Excel文件
            file_path = os.path.join(self.config["base_dir"], "match_by_simcse/data/supplier_type.xlsx")
            
            if not os.path.exists(file_path):
                self.logger.warning(f"供应商映射文件不存在: {file_path}")
                return {}
            ...
            self.logger.info(f"使用默认供应商映射: {len(default_mapping)} 个")
    ...
    def match_room_types(
        ...
        try:
            ...
            for i, (room, text) in enumerate(zip(spl_rooms, spl_texts)):
                self.logger.debug(f"SPL房型[{i}] ID:{room.get('RoomTypeID', '')}, 原文本:{room.get('RoomTypeName', '')}, 处理后:{text}")
            
            for i, (room, text) in enumerate(zip(s_rooms, s_texts)):
                self.logger.debug(f"标准房型[{i}] ID:{room.get('SRoomID', '')}, 原文本:{room.get('RoomTypeName', '')}, 处理后:{text}")
            ...
            for i, spl_room in enumerate(spl_rooms):
                ...
                self.logger.debug(f"房型匹配 - SPL房型ID: {spl_room.get('RoomTypeID', '')}, 原匹配ID: {original_match}, 最佳匹配ID: {best_match}, 分数: {best_similarity:.4f}, 状态: {match_status}")
                ...
                result = {
                    ...
                    "酒店名称": spl_room.get("HotelName", ""),
                    "SPL房型": spl_room.get('RoomTypeName', ''),
                    "SPL房型中文": spl_room.get('RoomTypeNameCN', ''),
                    "SPL床型": spl_room.get('BedTypeDesc', ''),
                    "SPL床型中文": spl_room.get('BedTypeDescCN', ''),
                    "标准房型": s_room.get('RoomTypeName', ''),
                    "标准房型中文": s_room.get('RoomTypeNameCN', ''),
                    "标准床型": s_room.get('BedTypeDesc', ''),
                    "标准床型中文": s_room.get('BedTypeDescCN', ''),
...
class BatchProcessor:
    ...
    def __init__(self, config: Dict):
        ...
        self.matcher = RoomTypeMatcher(config["match_api_url"], config["model_path"], config)
        
        # 确保输出目录存在
        self.output_dir = os.path.join(config["base_dir"], config["output_dir"])
    ...
    def process_hotel(self, hotel_id: str, hotel_name: str = "未知", country: str = "未知") -> Optional[Dict]:
        ...
        try:
            ...
            original_matched = sum(1 for room in spl_rooms if room.get("SRoomID", 0) > 0)
            self.stats["matched_rooms"] += original_matched
            self.stats["unmatched_rooms"] += len(spl_rooms) - original_matched
            
            # 执行匹配
            match_result = self.matcher.match_room_types(spl_rooms, s_rooms, self.config["threshold"])
            ...
            # 分类匹配结果
            new_matches = [m for m in matches if m["匹配状态"] == "原未匹配" and m["预测结果"] == 1]
            changed_matches = [m for m in matches if m["匹配状态"] == "匹配变化" and m["预测结果"] == 1]
            kept_matches = [m for m in matches if m["匹配状态"] == "已匹配" and m["预测结果"] == 1]
            ...
            self.stats["kept_matches"] = self.stats.get("kept_matches", 0) + len(kept_matches)
            self.stats["model_matched"] += len(new_matches) + len(changed_matches) + len(kept_matches)
            self.stats["model_unmatched"] += len(unmatched)
            
            # 计算匹配率
            if self.stats["unmatched_rooms"] > 0:
                self.stats["model_match_rate"] = (self.stats["new_matches"] / self.stats["unmatched_rooms"]) * 100
            
            self.logger.info(f"酒店 {hotel_id} 处理完成: {len(matches)} 个匹配结果, {len(new_matches)} 个新匹配, {len(changed_matches)} 个变化, {len(kept_matches)} 个保持匹配")
    ...
    def process_hotels(self, hotels_data: pd.DataFrame):
        ...
        try:
            ...
            output_base = os.path.join(self.output_dir, f"batch_results_{timestamp}")
            ...
            for idx, row in hotels_data.iterrows():
                ...
                self.logger.info(f"处理进度: {idx+1}/{len(hotels_data)} - 酒店: {hotel_id}")
    ...
    def _save_results(self, all_matches: List[Dict], new_matches: List[Dict], 
        ...
        try:
            # 转换为DataFrame
            all_df = pd.DataFrame(all_matches) if all_matches else pd.DataFrame()
            new_df = pd.DataFrame(new_matches) if new_matches else pd.DataFrame()
            changed_df = pd.DataFrame(changed_matches) if changed_matches else pd.DataFrame()
            kept_df = pd.DataFrame(kept_matches) if kept_matches else pd.DataFrame()
            unmatched_df = pd.DataFrame(unmatched) if unmatched else pd.DataFrame()
            failed_df = pd.DataFrame(failed_hotels) if failed_hotels else pd.DataFrame()
            ...
            # 保存结果
            if not all_df.empty:
                all_df.to_csv(f"{output_base}_all{suffix}.csv", index=False, encoding='utf-8')
            if not new_df.empty:
                new_df.to_csv(f"{output_base}_new{suffix}.csv", index=False, encoding='utf-8')
            if not changed_df.empty:
                changed_df.to_csv(f"{output_base}_changed{suffix}.csv", index=False, encoding='utf-8')
            if not kept_df.empty:
                kept_df.to_csv(f"{output_base}_kept{suffix}.csv", index=False, encoding='utf-8')
            if not unmatched_df.empty:
                unmatched_df.to_csv(f"{output_base}_unmatched{suffix}.csv", index=False, encoding='utf-8')
            if not failed_df.empty:
                failed_df.to_csv(f"{output_base}_failed{suffix}.csv", index=False, encoding='utf-8')
            
            self.logger.info(f"已保存{'最终' if is_final else '中间'}结果到: {output_base}")
    ...
    def _log_final_stats(self):
        ...
        # 添加匹配数据验证信息
        model_matches_sum = self.stats['new_matches'] + self.stats['changed_matches'] + kept_matches_count
        self.logger.info(f"模型匹配数验证: {model_matches_sum} (新增 + 变化 + 保持)")
        
        if model_matches_sum != self.stats['model_matched']:
            self.logger.warning(f"模型匹配数不一致: model_matched={self.stats['model_matched']}, 计算和={model_matches_sum}")
        
        if self.stats['unmatched_rooms'] > 0:
            new_match_rate = (self.stats['new_matches'] / self.stats['unmatched_rooms']) * 100
...
def main():
    ...
    parser.add_argument('--hotels', type=str, default='/home/maxon/disk2/roomMatch/room_match/match_by_simcse/data/1000sampled_hotels.csv', help='酒店数据文件路径')
    parser.add_argument('--country', type=str, default=None, help='指定处理的国家代码')
    parser.add_argument('--max_hotels', type=int, default=1000, help='最大处理酒店数')
    parser.add_argument('--threshold', type=float, default=0.9, help='匹配阈值')
    parser.add_argument('--model', type=str, default='/home/maxon/disk2/roomMatch/room_match/match_by_simcse/checkpoints_6_lesslr', help='模型路径')
    parser.add_argument('--output', type=str, default='/home/maxon/disk2/roomMatch/room_match/match_by_simcse/output', help='输出目录')
    ...
    try:
        ...
        # 设置日志
        log_dir = os.path.join(config["base_dir"], config["output_dir"], "logs")
