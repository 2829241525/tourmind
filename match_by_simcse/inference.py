...
MODEL_PATH = os.path.join(BASE_DIR, 'checkpoints_roberta_ppt/best_model')
...
# 输出文件配置
OUTPUT_FILES = {
    'best_results': 'best_results_merged.csv',          # 最佳结果文件
    'error_cases': 'error_cases_merged.csv',     # 错误案例文件
    # 'best_results': 'best_results.csv',          # 最佳结果文件
    # 'error_cases': 'error_cases.csv',
    'error_analysis': 'error_analysis_merged.json',     # 错误分析结果
    'metrics': 'evaluation_metrics_merged.json'         # 评估指标结果
}
...
THRESHOLD_RANGE = [0.5, 0.6, 0.7, 0.8, 0.9, 0.92, 0.94, 0.96, 0.98]  # 阈值搜索范围
...
class SimCSEPredictor:
    def __init__(self, model_path: str):
        ...
        try:
            ...
            logger.error(f"请确保模型路径 '{model_path}' 有效，并且包含所有必要的模型文件")
            logger.error(
                "如果在离线环境中，请参考 https://huggingface.co/docs/transformers/installation#offline-mode")
            raise
    ...
    def predict(self, test_file, batch_size=BATCH_SIZE):
        """预测模型结果并返回相似度，不进行阈值处理"""
        # 加载测试数据
        ...
        # 批量处理数据
        for i in tqdm(range(0, len(test_data), batch_size), desc="预测进度"):
            batch = test_data.iloc[i:i + batch_size]

            try:
                # 编码文本
                source_embeddings = self.encode_batch(
                ...
                # 收集结果数据
                batch_data = {
                    'spl_room_type_id': batch['spl_room_type_id'].tolist(),
                    'source_text': batch['spl_room_text'].tolist(),
                    'target_text': batch['s_room_text'].tolist(),
                    'similarity': pair_similarities.numpy(),
        ...
        return results_df, similarity_stats


def evaluate_with_threshold(results_df, threshold):
    """使用给定阈值评估预测结果"""
    # 根据阈值生成预测结果
    predictions = (results_df['similarity'] > threshold).astype(int)
    ...
    # 添加预测结果到DataFrame的副本中
    result_with_predictions = results_df.copy()
    result_with_predictions['prediction'] = predictions

    return metrics, result_with_predictions
...
def main():
    ...
    # 只进行一次模型预测，获取相似度
    print("\n正在进行模型预测...")
    results_df, similarity_stats = inferencer.predict(TEST_FILE)
    ...
    for threshold in THRESHOLD_RANGE:
        print(f"\n使用阈值 {threshold:.2f}:")
        metrics, results_with_predictions = evaluate_with_threshold(
        ...
        if current_score > best_score:
            ...
            best_results = results_with_predictions

    # 将相似度统计添加到最佳指标中
    best_metrics['similarity_stats'] = similarity_stats
    ...
    # 分析错误案例
    print("\n分析错误案例...")
    error_cases = best_results[best_results['prediction']
                               != best_results['true_label']]
    ...
    # 分析相似度分布
    print("\n相似度分布分析:")
    positive_similarities = best_results[best_results['true_label']
                                         == 1]['similarity']
    negative_similarities = best_results[best_results['true_label']
                                         == 0]['similarity']
