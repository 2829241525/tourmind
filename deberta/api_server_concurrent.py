...
class SimCSEPredictor:
    def __init__(self, model_path: str):
        ...
        if torch.cuda.is_available():
            ...
            if gpu_cap[0] < 7:  # 7.0 = Volta 架构
                self.use_fp16 = False
                logger.warning(f"当前GPU架构 ({gpu_cap[0]}.{gpu_cap[1]}) 不支持高效的FP16，将使用FP32")
            else:
    ...
    def predict(self, source_texts: List[str], target_texts: List[str], threshold: float = DEFAULT_THRESHOLD):
        ...
        for i, (source, target) in enumerate(zip(source_texts, target_texts)):
            # 创建缓存键
            cache_key = hashlib.md5(f"{source}:::{target}:::{threshold}".encode()).hexdigest()
        ...
        if uncached_sources:
            ...
            uncached_similarities = self.calculate_similarity(logits).cpu().numpy().tolist()
        ...
        # 根据阈值进行预测
        predictions = [1.0 if sim > threshold else 0.0 for sim in result_similarities]
...
async def health_check(token: str = None):
    ...
    if global_state.cleaner is None:
        ...
        cleaner_info["rule_count"] = sum(len(rules) for rules in global_state.cleaner.room_rules.values())
    ...
    if predictor:
        ...
        if hasattr(predictor, 'total_processing_time') and hasattr(predictor, 'total_requests'):
            if predictor.total_requests > 0:
                avg_time = (predictor.total_processing_time / predictor.total_requests) * 1000  # ms
                stats["performance"]["avg_request_time_ms"] = round(avg_time, 2)
            stats["performance"]["total_requests"] = predictor.total_requests
            
        # 内存使用统计
        if torch.cuda.is_available():
            stats["performance"]["memory_usage_mb"]["allocated"] = round(torch.cuda.memory_allocated() / (1024 * 1024), 2)
            stats["performance"]["memory_usage_mb"]["reserved"] = round(torch.cuda.memory_reserved() / (1024 * 1024), 2)
