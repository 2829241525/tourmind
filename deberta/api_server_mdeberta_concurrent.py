...
import math  # 导入math模块用于处理无穷值和NaN值
...
if MODEL_PATHS_JSON:
    try:
        ...
        MODEL_PATHS = {
            'mdeberta': os.path.join(BASE_DIR, 'checkpoints_mdeberta_result/best_model')
        }
else:
    MODEL_PATHS = {
        'mdeberta': os.path.join(BASE_DIR, 'checkpoints_mdeberta_result/best_model')
        ...
        while True:
            ...
            try:
                ...
                while len(batch_items) < self.max_batch_size and time.time() - start_time < self.timeout:
                    try:
                        item_id, item = self.queue.get(block=True, timeout=self.timeout)
...
async def lifespan(app: FastAPI):
    ...
    logger.info(f"日志文件: {LOG_FILE}，最大大小: {MAX_LOG_SIZE/1024/1024/1024:.1f}GB，保留备份数: {BACKUP_COUNT}")
...
class SimCSEPredictor:
    def __init__(self, model_path: str):
        ...
        # 尝试加载配置文件
        config_dir = os.path.join(os.path.dirname(os.path.dirname(model_path)), 'config')
        ...
        # 始终使用FP32精度，不使用FP16
        self.use_fp16 = False
        logger.info("始终使用FP32全精度运算，不使用半精度")
        ...
        if torch.cuda.is_available():
            ...
            logger.info(f"已清理GPU缓存，当前占用: {torch.cuda.memory_allocated()/1024/1024:.2f}MB，缓存: {torch.cuda.memory_reserved()/1024/1024:.2f}MB")
    ...
    async def predict_async(self, source_texts: List[str], target_texts: List[str], threshold: float = DEFAULT_THRESHOLD):
        """异步预测文本对的匹配情况，使用批处理处理所有请求"""
        if len(source_texts) != len(target_texts):
            raise ValueError("源文本和目标文本数量必须相同")
        
        # 直接批量处理所有文本对，而不是一对一对处理
        batch_size = MAX_BATCH_SIZE
        results = []
        
        start_time = time.time()
        total_pairs = len(source_texts)
        logger.info(f"开始批量处理 {total_pairs} 对文本，批次大小: {batch_size}，使用FP32全精度")
        ...
        for i in range(0, len(source_texts), batch_size):
            ...
            logger.debug(f"处理第 {i//batch_size + 1} 批，共 {current_batch_size} 对文本")
            
            # 直接使用encode_text_pair批量编码
            logits = self.encode_text_pair(batch_sources, batch_targets)
            similarities = self.calculate_similarity(logits).cpu().numpy()
            
            batch_process_time = time.time() - batch_start_time
            logger.debug(f"批处理完成，耗时: {batch_process_time:.3f}秒，速度: {current_batch_size/batch_process_time:.1f}对/秒")
            
            # similarities已经是numpy数组，直接转为列表
            ...
            # 构建每个文本对的结果
            for j, similarity in enumerate(similarities_list):
                similarity_val = float(similarity)
                prediction = 1.0 if similarity_val > threshold else 0.0
                
                results.append({
                    'similarities': [similarity_val],
                    'predictions': [prediction]
                })
        ...
        logger.info(f"批量处理完成，共 {total_pairs} 对文本，匹配 {matched_pairs} 对，总耗时: {total_time:.3f}秒，平均速度: {total_pairs/total_time:.1f}对/秒")
    ...
    try:
        with model_lock:
            ...
            try:
                ...
                logger.info(f"模型加载成功，类型：{predictor.loss_type}，耗时：{load_time:.2f}秒")
            except Exception as e:
...
def init_model(model_type=None):
    ...
    if not model_type:
        ...
        if model_type:
            logger.info(f"从环境变量获取模型类型: {model_type}")
    ...
    with model_lock:
        ...
        if model_path is None:
            ...
            if os.path.exists(possible_path):
                ...
                logger.info(f"模型路径成功构建: {model_path}")
            else:
                logger.error(f"错误：未找到模型路径，类型: {model_type}，已检查路径: {possible_path}")
                ...
                if available_paths:
                    ...
                    logger.warning(f"找不到请求的模型: {model_type}，将使用可用模型: {model_name}")
                else:
...
async def match_rooms(request: MatchRequest, background_tasks: BackgroundTasks):
    ...
    try:
        ...
        # 记录请求输入数据
        request_id = time.strftime("%Y%m%d%H%M%S") + str(int(time.time() * 1000) % 1000)
        ...
        if predictor.model_path != model_path and not global_state.model_loading:
            # 记录要切换的模型
            logger.info(f"请求ID: {request_id} - 需要切换模型: {model_type} ({model_path})")
            
            # 在后台加载新模型
            background_tasks.add_task(init_model, model_type)
            
            # 返回使用当前可用模型的信息
            logger.info(f"请求ID: {request_id} - 继续使用当前模型处理请求：{predictor.model_path}")
        ...
        for spl_room, spl_bed in zip(
            ...
            source_text = process_text_with_prefix(None, spl_room, spl_bed, True, cleaner)
        ...
        for s_room, s_bed in zip(
            ...
            target_text = process_text_with_prefix(None, s_room, s_bed, False, cleaner)
        ...
        # 记录处理结果
        logger.info(f"请求ID: {request_id} - 匹配完成，共处理 {len(source_texts)} 对文本，预测匹配 {sum(result['predictions'])} 对")
...
if __name__ == "__main__":
    ...
    uvicorn.run(
        "api_server_mdeberta_concurrent:app",  # 使用导入字符串
        host="0.0.0.0", 
        port=13003,
