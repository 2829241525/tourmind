    ...
    async def periodic_memory_cleanup():
        while True:
            ...
            try:
                ...
                logger.warning(f"定期内存清理失败: {str(e)}")
...
class SimCSEPredictor:
    ...
    def __del__(self):
        ...
        try:
            ...
            logger.info("SimCSEPredictor资源已释放")
        except Exception as e:
            logger.warning(f"析构SimCSEPredictor时出错: {str(e)}")
        ...
        for i in range(0, len(texts), batch_size):
            ...
            with torch.no_grad():
                with autocast(enabled=self.use_fp16):
                    ...
                    # 显式删除GPU张量释放显存
                    del batch_embeddings, outputs, encoded
                    if torch.cuda.is_available():
                        torch.cuda.empty_cache()
    ...
    def encode_text_pair(self, text1_list, text2_list, batch_size=256):
        ...
        for i in range(0, len(text1_list), batch_size):
            ...
            with torch.no_grad():
                ...
                # 清理当前批次的GPU内存
                del encoded, outputs, batch_logits
                if torch.cuda.is_available():
                    torch.cuda.empty_cache()
    ...
    def _predict_in_thread(self, source_texts, target_texts, threshold):
        ...
        with torch.no_grad():
            ...
            # 转换为CPU并获取结果，然后显式删除GPU张量
            similarities_cpu = similarities.cpu().numpy().tolist()
            predictions_cpu = predictions.cpu().numpy().tolist()
    ...
    try:
        with model_lock:
            ...
            if old_predictor is not None:
                logger.info("释放旧模型的GPU显存...")
                try:
                    # 删除旧模型的引用
                    del old_predictor.model
                    del old_predictor.tokenizer
                    del old_predictor
...
if __name__ == "__main__":
    ...
    uvicorn.run(
        "api_server_concurrent_copy:app",  # 使用导入字符串
        host="0.0.0.0",
        port=13009,
