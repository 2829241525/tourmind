...
OUTPUT_MATCH_FILE = os.path.join(
    PROJECT_ROOT, "room_match/data/processed_country_data.csv")
OUTPUT_TRIPLET_FILE = os.path.join(
    PROJECT_ROOT, "room_match/data/processed_country_data_triplet.csv")
...
CHINESE_REMOVE_PATTERN = re.compile(
    r'[\u4e00-\u9fff\u3000-\u303f\uff00-\uffef]+')
...
def process_text_with_prefix(prefix: str, country_prefix: str, room_name: str, bed_name: str, is_supplier: bool = False) -> str:
    # 提取英文
    room = extract_english(room_name)
    bed = extract_english(bed_name)

    prefix_text = prefix if prefix else UNKNOWN_TEXT
    country_prefix_text = country_prefix if country_prefix else UNKNOWN_TEXT
    ...
    if country_prefix:
        parts.append(
            f"{COUNTRY_PREFIX} {country_prefix_text}" if country_prefix_text else "")
    if room:
...
def process_data_parallel(df: pd.DataFrame, func, is_supplier: bool, desc: str) -> pd.Series:
    # 获取CPU核心数，预留一个核心给系统
...
def process_supplier_text(row):
    return process_text_with_prefix(row['supplier_name'],
                                    row['source_file'],
                                    row['spl_room_name'],
...
def process_source_text(row):
    return process_text_with_prefix(None,
                                    None,
                                    row['s_room_name'],
                                    row['s_room_bed_name'],
                                    is_supplier=True)
...
def process_data(df: pd.DataFrame) -> Tuple[pd.DataFrame]:
    total_rows = len(df)
    print("\n开始处理数据...")
    ...
    # 处理supplier端数据
    print("\n处理supplier端数据...")
    df['spl_room_text'] = process_data_parallel(
        df, process_supplier_text, True, "supplier处理进度")

    # 处理source端数据
    print("\n处理source端数据...")
    df['s_room_text'] = process_data_parallel(
        df, process_source_text, True, "source处理进度")

    # # 处理negative数据
    # print("\n处理negative数据...")
    # df['negative'] = process_data_parallel(df, process_negative_text, False, "negative处理进度")

    # 创建匹配数据框
    df_match = pd.DataFrame({
        's_hotel_id': df['s_hotel_id'],
        'supplier_name': df['supplier_name'],
        'country': df['source_file'],
