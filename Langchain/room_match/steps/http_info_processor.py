...
class HttpInfoProcessor:
    ...
    def process_http_request(self, url: str, method: str = "GET", headers: Optional[Dict] = None, **kwargs) -> Dict[str, Any]:
        ...
        try:
            ...
            response = requests.request(method=method, url=url, headers=default_headers, verify=False, **kwargs)
        ...
        try:
            ...
            for i, pair in enumerate(matched_pairs):
                ...
                if normalized_pair_key not in seen_pairs:
                    ...
                    # 计算Jaccard相似度
                    jaccard_sim = self.calculate_jaccard_similarity(normalized_supplier, normalized_standard)
            ...
            for standard_room in standard_room_groups:
                standard_room_groups[standard_room].sort(key=lambda x: x['jaccard_similarity'])
            
            logger.info(f"去重后共{len(similarity_data)}个唯一配对，涵盖{len(standard_room_groups)}种标准房型")
            ...
            if len(standard_room_groups) <= 20:
                ...
                selected_standard_rooms = random.sample(list(standard_room_groups.keys()), 20)
            ...
            avg_similarity = sum(item['jaccard_similarity'] for item in selected_examples) / len(selected_examples) if selected_examples else 0
            logger.info(f"使用Jaccard相似度选择{len(formatted_examples)}个案例，平均相似度: {avg_similarity:.3f}")
            ...
            for pair in matched_pairs:
                ...
                if normalized_pair_key not in seen_pairs:
                    ...
                    standard_room_groups[normalized_standard].append(normalized_pair_key)
    ...
    def extract_room_types(self, response_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"响应数据结构: {list(response_data.keys()) if isinstance(response_data, dict) else 'Not a dict'}")
            ...
            for room in s_room_type_list:
                ...
                if room_id:
                    ...
                    standard_rooms_dict[room_id] = {"combined_name": combined_name, "room_id": room_id}
            ...
            for room in s_room_type_list:
                ...
                combined_name = f"{room_name} {bed_desc}".strip()
                if combined_name:
                    standard_room_types.append({"combined_name": combined_name})
            ...
            logger.info(f"房型数据提取完成: 未匹配{len(unmatched_supplier_rooms)}个, 标准房型{len(standard_room_types)}个, 已匹配{len(matched_pairs)}个")
    ...
    def format_room_types_for_prompt(self, extracted_data: Dict[str, Any]) -> Dict[str, List[str]]:
        ...
        try:
            ...
            matched_examples = self.get_matched_examples_by_similarity(matched_pairs)
            
            # 2. 未匹配的供应商房型 (格式: [1]: 房型名称)
            unmatched_rooms = extracted_data.get("unmatched_supplier_rooms", [])
            unmatched_list = []
            ...
            if self.dl_matcher and unmatched_room_names and standard_room_names:
                try:
                    ...
                    if dl_similarity_result["status"] == "success":
                        ...
                        # 获取最佳匹配
                        best_matches = self.dl_matcher.get_best_matches(threshold=0.7)
                        if best_matches:
                            logger.info(f"发现{len(best_matches)}个高质量匹配（相似度≥0.7）:")
                            for match in best_matches[:5]:  # 只显示前5个
                                logger.info(f"  {match['supplier_room']} → {match['standard_room']} (相似度: {match['similarity']:.4f})")
                    else:
                        logger.warning(f"DL匹配失败: {dl_similarity_result.get('error', 'Unknown error')}")
    ...
    def generate_room_matching_with_agent(self, formatted_data: Dict[str, List[str]]) -> Dict[str, Any]:
        ...
        try:
            ...
            agent_executor = initialize_agent(
                ...
                max_iterations=self.room_matching_config.get('agent_max_iterations', 4),  # 从3增加到10
                handle_parsing_errors=self.room_matching_config.get('agent_handle_parsing_errors', True),
                early_stopping_method=self.room_matching_config.get('agent_early_stopping_method', 'generate')
            )
    ...
    def process_room_matching(self, formatted_data: Dict[str, List[str]]) -> Dict[str, Any]:
        ...
        Returns:
            包含匹配结果的字典
        """
        matching_method = self.room_matching_config.get('matching_method', 'agent')
        
        try:
            if matching_method == 'llm':
                logger.info("使用LLM直接调用方式进行房型匹配")
                result = self.generate_extraction_rules_with_llm(formatted_data)
                # 标准化返回格式
                return {
                    "status": "success",
    ...
    def execute_step(self, hotel_id: str = None, url: str = None, method: str = "GET", 
        ...
        try:
            ...
            response_data = self.process_http_request(url=final_url, method=method, headers=final_headers, **kwargs)
